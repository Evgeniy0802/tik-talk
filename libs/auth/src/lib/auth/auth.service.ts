...
export class AuthService {
	...
	get isAuth() {
		//будем проверять авторизован пользователь или нет
		if (!this.token) {
	...
	login(payload: { username: string; password: string }) {
		...
		return this.http
			.post<TokenResponse>(`${this.baseApiUrl}token`, fd) //когда мы делаем post запрос body запрос передаётся как второй аргуменят (payload) или fd
			.pipe(
