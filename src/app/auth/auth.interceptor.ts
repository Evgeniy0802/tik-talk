...
const refreshAndProceed = (
	...
	if (!isRefreshing$.value) {
		...
		return authService.refreshAuthToken().pipe(
			//рефреш токен это стрим, нам нужно перейти в другой стрим, тот запрос с которого мы начали, с полученными данными мы должны отправить запрос еще туда
			switchMap((res) => {
				//берёт стрим который ты ему дал и меняет на другой стрим
...
const addToken = (req: HttpRequest<any>, token: string) => {
	return req.clone({
		//клонируем наш реквест и видоимзеняем его
		setHeaders: {
